...
(function() {
  ...
  window.TechPackV11.BulkStep4 = {
    ...
    /**
     * Check if garment has complete colorway (single colorway per garment)
     */
    validateGarmentComplete: function(garment) {
      if (!garment.colorway) return false;
      return this.calculateColorwayTotal(garment.colorway) > 0;
    },

    /**
     * Apply quick fill preset to garment's colorway
     ...
     */
    applyQuickFill: function(garmentId, preset, total) {
      const garment = this.garments.find(g => g.id === garmentId);
      if (!garment || !garment.colorway) return;
      ...
      // Update garment's colorway sizes
      garment.colorway.sizes = distribution;
     ...
     */
    savePreset: function(garmentId) {
      const garment = this.garments.find(g => g.id === garmentId);
      if (!garment || !garment.colorway) {
        alert('No colorway to save as preset');
        return;
      }

      const total = this.calculateColorwayTotal(garment.colorway);
      ...
      localStorage.setItem('v11_bulk_size_preset', JSON.stringify(garment.colorway.sizes));

      // Show feedback
      alert(`✓ Preset saved!\n\nTotal: ${total} pcs\n${Object.entries(garment.colorway.sizes).map(([size, qty]) => `${size}: ${qty}`).join(', ')}`);

      console.log('[BulkStep4] Saved preset:', garment.colorway.sizes);
    },

    /**
     * Load preset into garment's colorway
     */
    loadPreset: function(garmentId) {
      ...
      garment.colorway.sizes = JSON.parse(preset);
    ...
    /**
     * Set colorway for garment (single colorway per garment)
     */
    addColorwayToGarment: function(garmentId, colorData) {
      ...
      // Check if garment already has a colorway - return it to Bulk Color Cart if Pantone
      if (garment.colorway && garment.colorway.colorType === 'pantone' && garment.colorway.pantoneColor) {
        console.log('[BulkStep4] Replacing existing Pantone color, returning to Bulk Color Cart:', garment.colorway.pantoneColor);

        const labDip = {
          id: 'labdip-' + Date.now(),
          code: garment.colorway.pantoneColor.code,
          name: garment.colorway.pantoneColor.name,
          hex: garment.colorway.hex,
          washTechnique: garment.colorway.pantoneColor.washTechnique || 'none'
        };
      ...
      // Set garment's single colorway with empty sizes
      garment.colorway = {
      ...
      console.log('[BulkStep4] Set colorway for garment:', garment.type, colorData.color);
    },

    /**
     * Clear colorway from garment (and return Pantone to Bulk Color Cart if applicable)
     */
    deleteColorway: function(garmentId) {
      const garment = this.garments.find(g => g.id === garmentId);
      if (!garment || !garment.colorway) return;

      if (confirm(`Delete ${garment.colorway.color} colorway?`)) {
        // Check if colorway is Pantone type - return to Bulk Color Cart
        if (garment.colorway.colorType === 'pantone' && garment.colorway.pantoneColor) {
          console.log('[BulkStep4] Returning Pantone color to Bulk Color Cart:', garment.colorway.pantoneColor);
        ...
        // Clear colorway
        garment.colorway = null;

        this.render();
        this.updateMOQProgress();

        console.log('[BulkStep4] Cleared colorway from garment');
      }
    },
    ...
    /**
     * Update size quantity for garment's colorway
     */
    updateColorwaySize: function(garmentId, size, quantity) {
      const garment = this.garments.find(g => g.id === garmentId);
      if (!garment || !garment.colorway) return;

      // Update size
      garment.colorway.sizes[size] = parseInt(quantity) || 0;

      // Update total display (no full re-render for performance)
      const totalEl = document.querySelector(`[data-garment-id="${garmentId}"] .colorway-total`);
      if (totalEl) {
        const total = this.calculateColorwayTotal(garment.colorway);
        totalEl.textContent = `${total} pcs`;
      }

      // Update State in-memory (NO auto-save) and update MOQ
      if (window.TechPackV11.State) {
        window.TechPackV11.State.formData.garments = this.garments;
      }
      this.updateMOQProgress();
    ...
    /**
     * Render individual garment with single colorway
     */
    renderGarmentColorwayCard: function(garment) {
      const colorway = garment.colorway; // Singular colorway

      // Check if garment is complete (has colorway with sizes)
      const isComplete = this.validateGarmentComplete(garment);
      ...
      // Quick Fill buttons (positioned below garment image on left side)
      const quickFillHTML = colorway ? `
        <div class="garment-quick-fill-buttons">
          <button type="button" class="quick-fill-btn quick-fill-btn--even" data-action="quick-fill" data-garment-id="${garment.id}" data-preset="even" title="Even Split">
          ...
          <button type="button" class="quick-fill-btn quick-fill-btn--bell" data-action="quick-fill" data-garment-id="${garment.id}" data-preset="bell" title="Bell Curve">
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
          ...
          <button type="button" class="quick-fill-btn quick-fill-btn--popular" data-action="quick-fill" data-garment-id="${garment.id}" data-preset="popular" title="Popular Sizes">
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
          ...
          <button type="button" class="quick-fill-btn quick-fill-btn--load" data-action="load-preset" data-garment-id="${garment.id}" title="Load Preset">
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
      ...
      // Colorway section (only if colorway exists)
      let colorwayHTML = '';
      if (colorway) {
        ...
        colorwayHTML = `
          <div class="colorway-inline-item">
            <div class="colorway-inline-header">
              <div class="colorway-inline-info">
                ...
                <span class="colorway-total">${total} pcs</span>
              </div>
              <button type="button" class="colorway-inline-delete" data-action="delete-colorway" data-garment-id="${garment.id}" title="Delete Colorway">
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
            ...
            <div class="preset-buttons">
              <button type="button" class="preset-btn preset-btn--save" data-action="save-preset" data-garment-id="${garment.id}" title="Save sizes as preset">
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
    ...
    renderLabDipCart: function() {
      ...
      if (totalEl) {
        totalEl.textContent = hasItems ? `${this.labDipCart.length} color${this.labDipCart.length !== 1 ? 's' : ''}` : '0 colors';
      }
    ...
    updateMOQProgress: function() {
      const totalQuantity = this.garments.reduce((sum, g) => {
        if (!g.colorway) return sum;
        return sum + this.calculateColorwayTotal(g.colorway);
      }, 0);
    ...
    updateOrderSummary: function() {
      ...
      this.garments.forEach(garment => {
        if (!garment.colorway) return;

        // Calculate total from single colorway
        const garmentTotal = this.calculateColorwayTotal(garment.colorway);
        totalPieces += garmentTotal;

        garmentSummaries.push({
          type: garment.type,
          fabric: garment.fabricType,
          colors: [{ hex: garment.colorway.hex, name: garment.colorway.color }],
          total: garmentTotal
        });
      });
