...
const V10_Registration = {
  ...
  initialize() {
    ...
    this.successModal = document.getElementById('registration-success-modal');
  ...
  updateVatValidation() {
    const countrySelect = document.getElementById('RegisterForm-Country');
    const vatInput = document.getElementById('RegisterForm-VatNumber');
    const vatWrapper = document.getElementById('vat-field-wrapper');
    const postalInput = document.getElementById('RegisterForm-PostalCode');
    const stateInput = document.getElementById('RegisterForm-State');

    if (!countrySelect || !vatInput || !vatWrapper) return;

    const selectedCountry = countrySelect.value;
    const isEuropean = ['PT', 'ES', 'FR', 'DE', 'IT', 'NL', 'BE', 'AT', 'SE', 'DK', 'FI', 'GB'].includes(selectedCountry);

    // Show/hide VAT field based on country
    if (isEuropean) {
      vatWrapper.classList.add('show');
      vatInput.setAttribute('required', 'required');
      vatInput.setAttribute('aria-required', 'true');
      vatInput.placeholder = 'Enter your VAT number';
    } else {
      vatWrapper.classList.remove('show');
      vatInput.removeAttribute('required');
      vatInput.setAttribute('aria-required', 'false');
      vatInput.value = ''; // Clear value when hidden
      this.clearFieldError(vatInput); // Clear any validation errors
    }
  ...
  initializeSuccessModal() {
    ...
    const continueButton = document.getElementById('success-modal-continue');
