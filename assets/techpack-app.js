(function() {
  ...
  // Pantone Color Management System
  const PantoneManager = {
    colors: null,
    isLoaded: false,

    // Load Pantone database from JSON file
    async loadDatabase() {
      if (this.isLoaded && this.colors) {
        return this.colors;
      }
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(`Failed to load Pantone database: ${response.status}`);
        }
        
        const pantoneData = await response.json();
        
        // Convert to array format for easier searching
        this.colors = Object.entries(pantoneData).map(([code, data]) => ({
          code: `PANTONE ${code}`,
          hex: `#${data.hex}`,
          name: data.name,
          searchCode: code
        ...
        this.isLoaded = true;
        debugSystem.log(`✅ Pantone database loaded: ${this.colors.length} colors`);
        return this.colors;
        
      } catch (error) {
        debugSystem.log('❌ Failed to load Pantone database', error, 'error');
        // Fallback to basic colors if database fails to load
        this.colors = this.getFallbackColors();
        this.isLoaded = true;
        return this.colors;
      }
    },

    // Get fallback colors if database loading fails
    getFallbackColors() {
      return [
        { code: 'PANTONE Black C', hex: '#000000', name: 'black', searchCode: 'black-c' },
        { code: 'PANTONE White', hex: '#FFFFFF', name: 'white', searchCode: 'white' },
        { code: 'PANTONE Cool Gray 11 C', hex: '#54585A', name: 'cool-gray-11', searchCode: 'cool-gray-11-c' },
        { code: 'PANTONE 18-1664 TPX', hex: '#C5282F', name: 'true-red', searchCode: '18-1664' },
        { code: 'PANTONE 19-4052 TPX', hex: '#0F4C75', name: 'classic-blue', searchCode: '19-4052' }
      ];
    },

    // Find closest Pantone color match
    async findClosest(hexColor, count = 1) {
      const colors = await this.loadDatabase();
      ...
      const sortedColors = colorDistances.sort((a, b) => a.distance - b.distance);
      
      return count === 1 ? sortedColors[0] : sortedColors.slice(0, count);
    },
    ...
    async findByCode(code) {
      ...
      return colors.find(color => 
        color.code.toLowerCase().includes(code.toLowerCase()) ||
        color.searchCode.toLowerCase().includes(code.toLowerCase())
      );
    },
...
// ENHANCED: Robust navigation with comprehensive error handling
  ...
  class GarmentManager {
    ...
    setupColorwayEventListeners(colorway, garmentId, colorwayId) {
      ...
      if (pantoneButtons && colorPicker) {
        ...
        colorPicker.addEventListener('change', async () => {
          ...
          const closestPantones = await PantoneManager.findClosest(colorPicker.value, 2);
    ...
    // Find closest Pantone color match
    async findClosestPantoneColor(hexColor) {
      const closestColor = await PantoneManager.findClosest(hexColor, 1);
      
      if (closestColor) {
        return {
          code: closestColor.code,
          confidence: Math.max(Math.round((1 - (closestColor.distance / 441.67)) * 100), 65) // 441.67 = sqrt(3 * 255^2)
        };
      }
    ...
    async findClosestPantoneColors(hexColor, count = 5) {
      return await PantoneManager.findClosest(hexColor, count);
    }
...
// EXISTING: Keep your exact setupNavigationButtons method
    ...
    async buildColorwayRecords(clientData, submissionId, submissionDate) {
      ...
      document.querySelectorAll('.techpack-garment').forEach(async garment => {
        ...
        garment.querySelectorAll('.techpack-colorway').forEach(colorway => {
          ...
          const colorName = pantoneButton && pantoneButton.dataset.pantoneCode ? 
            ...
            await findClosestPantoneColor(hexColor);
...
setupInitialization();
  ...
  async function findClosestPantoneColor(hexColor) {
    const closestColor = await PantoneManager.findClosest(hexColor, 1);
    return closestColor ? closestColor.code : 'Unknown Color';
  }
  ...
  async function populateGarmentsReview() {
        ...
        if (colorways.length > 0) {
          ...
          colorways.forEach((colorway, colorIndex) => {
            ...
            if (selectedPantoneButton && selectedPantoneButton.dataset.pantoneCode) {
              ...
              colorName = await findClosestPantoneColor(color);
            }
