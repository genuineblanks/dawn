...
// Enhanced Form Initialization
  class FormInitializer {
    ...
    reorganizeRegisteredClientFields() {
      const form = document.querySelector('#techpack-step-1 form');
      if (!form) return;

      // Find the visible form groups
      ...
      const productionGroup = document.querySelector('#production-type')?.closest('.techpack-form__group');
      const deadlineGroup = document.querySelector('#deadline')?.closest('.techpack-form__group');
      const notesGroup = document.querySelector('#notes')?.closest('.techpack-form__group');

      if (!companyGroup || !emailGroup || !productionGroup || !deadlineGroup) return;

      // Remove existing row classes
      [companyGroup, emailGroup, productionGroup, deadlineGroup].forEach(group => {
        const row = group.closest('.techpack-form__row');
        if (row) {
          // Move group out of row and insert it before the row
      ...
      // Create new row structure for registered clients
      const newRow1 = document.createElement('div');
      newRow1.className = 'techpack-form__row';
      
      const newRow2 = document.createElement('div');
      newRow2.className = 'techpack-form__row';

      // Row 1: Company Name - Email Address
      newRow1.appendChild(companyGroup);
      newRow1.appendChild(emailGroup);

      // Row 2: Production Type - Project Deadline
      newRow2.appendChild(productionGroup);
      newRow2.appendChild(deadlineGroup);

      // Insert the new rows before the notes group (if it exists)
      if (notesGroup) {
        form.insertBefore(newRow1, notesGroup);
        form.insertBefore(newRow2, notesGroup);
      } else {
        // Insert before actions if notes don't exist
        const actions = form.querySelector('.techpack-form__actions');
        if (actions) {
          form.insertBefore(newRow1, actions);
          form.insertBefore(newRow2, actions);
        } else {
          form.appendChild(newRow1);
          form.appendChild(newRow2);
        }
      }

      debugSystem.log('Reorganized registered client fields into 2x2 grid', {
        row1: ['Company Name', 'Email Address'],
        row2: ['Production Type', 'Project Deadline']
      });
    }
    ...
    restoreOriginalFieldLayout() {
      // This method would restore the original HTML structure
      // Since it's complex to perfectly restore, we'll just ensure proper grouping
      const form = document.querySelector('#techpack-step-1 form');
      if (!form) return;

      // Find all form groups
      const clientNameGroup = document.querySelector('#client-name')?.closest('.techpack-form__group');
      const companyGroup = document.querySelector('#company-name')?.closest('.techpack-form__group');
      const emailGroup = document.querySelector('#email')?.closest('.techpack-form__group');
      const vatGroup = document.querySelector('#vat-ein')?.closest('.techpack-form__group');
      const phoneGroup = document.querySelector('#phone')?.closest('.techpack-form__group');
      const countryGroup = document.querySelector('#country')?.closest('.techpack-form__group');
      const productionGroup = document.querySelector('#production-type')?.closest('.techpack-form__group');
      const deadlineGroup = document.querySelector('#deadline')?.closest('.techpack-form__group');
      const notesGroup = document.querySelector('#notes')?.closest('.techpack-form__group');

      // Remove all groups from any existing rows
      [clientNameGroup, companyGroup, emailGroup, vatGroup, phoneGroup, countryGroup, productionGroup, deadlineGroup].forEach(group => {
        if (group) {
          const row = group.closest('.techpack-form__row');
          if (row) {
            row.parentNode.insertBefore(group, row);
      ...
      // Create original row structure: Client Name - Company Name
      if (clientNameGroup && companyGroup) {
        const row1 = document.createElement('div');
        row1.className = 'techpack-form__row';
        row1.appendChild(clientNameGroup);
        row1.appendChild(companyGroup);
        form.insertBefore(row1, emailGroup || productionGroup || notesGroup || form.querySelector('.techpack-form__actions'));
      }

      // Row 2: Email Address - VAT/EIN Number  
      if (emailGroup && vatGroup) {
        const row2 = document.createElement('div');
        row2.className = 'techpack-form__row';
        row2.appendChild(emailGroup);
        row2.appendChild(vatGroup);
        form.insertBefore(row2, phoneGroup || productionGroup || notesGroup || form.querySelector('.techpack-form__actions'));
      }

      // Row 3: Phone Number - Country
      if (phoneGroup && countryGroup) {
        const row3 = document.createElement('div');
        row3.className = 'techpack-form__row';
        row3.appendChild(phoneGroup);
        row3.appendChild(countryGroup);
        form.insertBefore(row3, productionGroup || notesGroup || form.querySelector('.techpack-form__actions'));
      }

      // Row 4: Production Type - Project Deadline
      if (productionGroup && deadlineGroup) {
        const row4 = document.createElement('div');
        row4.className = 'techpack-form__row';
        row4.appendChild(productionGroup);
        row4.appendChild(deadlineGroup);
        form.insertBefore(row4, notesGroup || form.querySelector('.techpack-form__actions'));
      }

      debugSystem.log('Restored original field layout for new clients');
    }
