...
class V10_StudioNavigator {
  ...
  updateStepActionsLayout() {
    ...
    // 🔥 MOBILE FIX: On mobile, clear any inline styles and let CSS handle layout
    if (isMobile) {
      stepActions.style.justifyContent = '';  // Clear inline styles
      stepActions.classList.remove('v10-step-actions--three-button');
      console.log('📱 MOBILE: Cleared inline styles, letting CSS handle layout');
      return; // Exit early, let mobile CSS take over
    }
    ...
    if (visibleButtonCount >= 3) {
      ...
      console.log(`✅ Applied three-button layout: ${visibleButtonCount} buttons visible on step ${currentStep} (DESKTOP)`);
      ...
      console.log(`✅ Applied single-button right-align layout: step ${currentStep} with 1 button (DESKTOP)`);
    } else {
      // 2 buttons or other cases: Use default flex layout
      stepActions.classList.remove('v10-step-actions--three-button');
      stepActions.style.justifyContent = 'space-between';
      console.log(`✅ Applied two-button layout: ${visibleButtonCount} buttons visible on step ${currentStep} (DESKTOP)`);
    }
  }
...
class V10_GarmentStudio {
  ...
  handleSampleSizeSelection(garmentId, size) {
    ...
    // COLOR STUDIO ATTENTION: Check if this garment needs color assignment after size is set
    // This is crucial for design-studio sample type to trigger the red pulsing animation
    this.checkColorRequirement(garmentId);
  }
...
function initializeStep3ImportanceModal() {
  ...
  function showStep3Modal() {
    ...
    if (messageEl) {
      if (requestType === 'quotation') {
        messageEl.innerHTML = '<p><strong>We will quote only what you specify in this step.</strong></p>';
      } else if (requestType === 'sample-request' || requestType === 'bulk-order-request') {
        messageEl.innerHTML = '<p><strong>We will produce only what you specify in this step.</strong></p>';
      } else {
        messageEl.innerHTML = '<p><strong>We will process only what you specify in this step.</strong></p>';
      }
    }
