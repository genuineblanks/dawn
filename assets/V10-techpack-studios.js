...
// ==============================================
// V10 GARMENT TAB MANAGER - Tab Navigation System
// ==============================================

class V10_GarmentTabManager {
  constructor() {
    this.activeGarmentId = null;
    this.tabs = new Map();
    this.tabsContainer = null;
    this.tabsHeaderContainer = null;
    this.garmentCountElement = null;
  ...
  initializeElements() {
    this.tabsContainer = document.getElementById('garment-tabs');
    this.tabsHeaderContainer = document.getElementById('garment-tabs-container');
    this.garmentCountElement = document.getElementById('garment-count');
    this.tabTemplate = document.getElementById('V10-garment-tab-template');
    
    if (!this.tabTemplate) {
      console.error('❌ V10-garment-tab-template not found');
    }
  }
  
  bindEvents() {
    // Handle tab clicks with delegation
    if (this.tabsContainer) {
      this.tabsContainer.addEventListener('click', (e) => {
        const tab = e.target.closest('.garment-tab');
        const closeBtn = e.target.closest('.garment-tab__close');
        
        if (closeBtn && tab) {
          // Handle tab close
          e.stopPropagation();
          this.removeGarmentTab(tab.dataset.garmentId);
        } else if (tab) {
          // Handle tab switch
          this.switchToTab(tab.dataset.garmentId);
        }
      });
    }
  }
  ...
  /**
   * Add or update a tab for a garment
   */
  addOrUpdateTab(garmentData) {
    try {
      const existingTab = this.tabs.get(garmentData.id);
      
      if (existingTab) {
        this.updateTabContent(garmentData);
      } else {
        this.createNewTab(garmentData);
      }
      
      this.updateTabsVisibility();
      this.updateGarmentCount();
      
      // Set as active if it's the only tab or newly created
      if (this.tabs.size === 1 || !existingTab) {
        this.switchToTab(garmentData.id);
      }
      
    } catch (error) {
      console.error('❌ Error adding/updating tab:', error);
    }
  }
   ...
   * Create a new tab
   */
  createNewTab(garmentData) {
    if (!this.tabTemplate || !this.tabsContainer) return;
    
    const clone = this.tabTemplate.content.cloneNode(true);
    const tab = clone.querySelector('.garment-tab');
    
    if (!tab) return;
    
    // Set tab data
    tab.dataset.garmentId = garmentData.id;
    
    // Update content
    this.updateTabElements(tab, garmentData);
    
    // Add to container
    this.tabsContainer.appendChild(clone);
    
    // Store reference
    this.tabs.set(garmentData.id, tab);
    
    console.log(`✅ Created tab for garment ${garmentData.id}`);
  }
  
  /**
   * Update tab content
   */
  updateTabContent(garmentData) {
    const tab = this.tabs.get(garmentData.id);
    if (tab) {
      this.updateTabElements(tab, garmentData);
    }
  }
  ...
  /**
   * Update tab elements with garment data
   */
  updateTabElements(tab, garmentData) {
    // Update number
    const numberEl = tab.querySelector('.garment-tab__number');
    if (numberEl) {
      numberEl.textContent = garmentData.number || '?';
    }
    
    // Update title
    const titleEl = tab.querySelector('.garment-tab__title');
    if (titleEl) {
      let title = 'Select garment type';
      
      if (garmentData.type) {
        title = garmentData.type;
        if (garmentData.fabric) {
          title += ` - ${garmentData.fabric}`;
        }
      }
      
      titleEl.textContent = title;
      titleEl.title = title; // Add tooltip for long titles
    }
    
    // Update status
    const statusEl = tab.querySelector('.garment-tab__status');
    if (statusEl) {
      const isComplete = garmentData.isComplete || (garmentData.type && garmentData.fabric);
      statusEl.textContent = isComplete ? 'Complete' : 'Incomplete';
      statusEl.className = `garment-tab__status ${isComplete ? 'garment-tab__status--complete' : 'garment-tab__status--incomplete'}`;
    }
  }
   ...
   */
  switchToTab(garmentId) {
    if (!garmentId || this.activeGarmentId === garmentId) return;
    
    try {
      // Hide current garment
      if (this.activeGarmentId) {
        this.hideGarment(this.activeGarmentId);
        this.setTabActive(this.activeGarmentId, false);
      }
      
      // Show new garment
      this.showGarment(garmentId);
      this.setTabActive(garmentId, true);
      
      this.activeGarmentId = garmentId;
      
      console.log(`🔄 Switched to garment tab: ${garmentId}`);
  ...
  showGarment(garmentId) {
    ...
    if (garmentCard) {
      garmentCard.style.display = 'block';
    }
  }
   ...
   * Hide garment by ID
   */
  hideGarment(garmentId) {
    const garmentCard = document.querySelector(`[data-garment-id="${garmentId}"]`);
    if (garmentCard) {
      garmentCard.style.display = 'none';
    }
  }
   ...
   */
  setTabActive(garmentId, active) {
    const tab = this.tabs.get(garmentId);
    if (tab) {
      tab.classList.toggle('garment-tab--active', active);
    }
  }
  ...
  removeGarmentTab(garmentId) {
    ...
    // Remove from Map
    this.tabs.delete(garmentId);
    
    // If this was the active tab, switch to another
    if (this.activeGarmentId === garmentId) {
    ...
    // Trigger garment removal in the main system
    if (window.V10_SYSTEM && window.V10_SYSTEM.garmentStudio && window.V10_SYSTEM.garmentStudio.garmentManager) {
      window.V10_SYSTEM.garmentStudio.garmentManager.removeGarment(garmentId);
    }
    
    console.log(`🗑️ Removed tab for garment ${garmentId}`);
  }
  
  /**
   * Update tabs container visibility
   */
  updateTabsVisibility() {
    if (this.tabsHeaderContainer) {
      const shouldShow = this.tabs.size > 1;
      this.tabsHeaderContainer.style.display = shouldShow ? 'block' : 'none';
      
      // Also hide all but first garment if tabs are not shown
      if (!shouldShow && this.tabs.size === 1) {
        const singleGarmentId = Array.from(this.tabs.keys())[0];
        this.showGarment(singleGarmentId);
        this.activeGarmentId = singleGarmentId;
      }
    }
  }
   ...
   */
  updateGarmentCount() {
    if (this.garmentCountElement) {
      const count = this.tabs.size;
      this.garmentCountElement.textContent = `${count} garment${count !== 1 ? 's' : ''}`;
    }
  }
  
  /**
   * Get active garment ID
   */
  getActiveGarmentId() {
    return this.activeGarmentId;
  }
...
class V10_GarmentStudio {
  constructor() {
    ...
    this.tabManager = new V10_GarmentTabManager();
  ...
  renderGarment(garmentData) {
    try {
      ...
      if (this.garmentsContainer) {
        ...
        // Add/update tab for this garment
        if (this.tabManager) {
          this.tabManager.addOrUpdateTab(garmentData);
        }
