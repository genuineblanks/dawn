    ...
    if (!modal) {
      ...
      modal.innerHTML = `
        <div class="v10-modal-content v10-color-picker-content" style="background: #1a1a1a !important;">
          ...
          <div class="v10-modal-body" style="padding: 24px;">
  ...
  renderColorPresets() {
    ...
    container.innerHTML = this.colorPresets.map(color => `
      <button type="button" class="v10-color-preset" 
              ...
              style="background: #2a2a2a; border: 2px solid #3a3a3a; border-radius: 10px; padding: 14px 10px; cursor: pointer; transition: all 0.2s;">
        <div class="v10-color-preset-swatch" style="background-color: ${color.hex}; width: 60px; height: 60px; border-radius: 8px; border: 2px solid #ffffff; box-shadow: 0 2px 6px rgba(0,0,0,0.4); margin: 0 auto 8px;"></div>
        <div class="v10-color-preset-name" style="color: #ffffff; font-size: 0.85rem; font-weight: 600; margin-bottom: 4px;">${color.name}</div>
        <div class="v10-color-preset-code" style="color: #6b7280; font-size: 0.75rem;">${color.pantone}</div>
      </button>
    `).join('');
    
    // Also make preset colors clickable for visual selection
    container.querySelectorAll('.v10-color-preset').forEach(btn => {
      btn.addEventListener('click', (e) => {
        const hex = btn.querySelector('.v10-color-preset-swatch').style.backgroundColor;
        this.updateVisualColorDisplay(hex);
      });
    });
  }
    ...
    const initializeTechPack = () => {
      try {
        if (document.getElementById('techpack-v10-step-3')) {
          ...
          // Initialize Quantity Studio for bulk orders
          if (requestType === 'bulk-order-request') {
            setTimeout(() => {
              console.log('🔧 Initializing Quantity Studio for bulk order');
              if (window.v10GarmentStudio && typeof window.v10GarmentStudio.populateQuantityStudio === 'function') {
                window.v10GarmentStudio.populateQuantityStudio();
                console.log('✅ Quantity Studio initialized on page load');
              } else if (window.v10TechPackSystem && window.v10TechPackSystem.garmentStudio) {
                window.v10TechPackSystem.garmentStudio.populateQuantityStudio();
                console.log('✅ Quantity Studio initialized via TechPackSystem');
              }
            }, 500); // Give time for garments to load from state
          }
