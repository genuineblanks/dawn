(function () {
    ...
    */
    ...
    if (debug.panel && debug.content) {
      ...
      clearButton.style.cssText =
        "position: absolute; top: 5px; right: 5px; background: #374151; color: white; border: none; padding: 0.25rem 0.5rem; border-radius: 3px; font-size: 0.6rem; cursor: pointer;";
  ...
  function showStepWithAnimation(stepNumber) {
    const currentStepEl = document.querySelector(
      `.techpack-step[data-step="${currentStep}"]`
    );
    const targetStepEl = document.querySelector(
      `.techpack-step[data-step="${stepNumber}"]`
    );
  ...
  function updateStepProgress() {
    ...
    const stepProgressSteps = Array.from(
      container.querySelectorAll(".techpack-progress__step")
    );

    // Compute 0-based index & percent
    const zeroIndex = currentStep - 1;
    const totalSteps = stepProgressSteps.length;
    const pct = totalSteps > 1 ? (zeroIndex / (totalSteps - 1)) * 100 : 0;
    ...
    stepProgressSteps.forEach((el, i) => {
      setTimeout(() => {
        el.classList.toggle(
          "techpack-progress__step--completed",
          i < zeroIndex
        );
  ...
  function getColorwayCount() {
    const colorways = document.querySelectorAll(
      ".techpack-colorway[data-colorway-id]"
    );
  ...
  function getTotalQuantityFromAllColorways() {
    let total = 0;
    const colorwayInputs = document.querySelectorAll(
      '.techpack-size-grid__input[type="number"]'
    );
  ...
  function updateTotalQuantityDisplay(
    ...
    const totalQuantityElement = document.querySelector(
      "#total-quantity, .total-quantity-value, .techpack-total-quantity"
    );
    ...
    const minTextElement = document.querySelector(
      "#min-text, .total-quantity-text, [data-quantity-text]"
    );
    if (minTextElement) {
      let newText;
      if (colorwayCount === 1) {
  ...
  function calculateAndUpdateProgress() {
    ...
    console.log(
      `Quantity Progress: Colorways: ${colorwayCount}, Total: ${totalQuantity}, Min Required: ${minimumRequired}, Progress: ${percentage.toFixed(
        1
      )}%`
    );
  ...
  function updateQuantityProgressBar(percentage) {
    ...
    if (quantityProgressBar) {
      ...
      quantityProgressBar.style.transition =
        "width 0.5s ease-out, background-color 0.3s ease";
    ...
    if (tracker) {
      ...
      tracker.classList.toggle(
        "techpack-quantity-tracker--complete",
        isComplete
      );
  ...
  function updateColorwayValidationMessages() {
    ...
    colorways.forEach((colorway) => {
      ...
      if (!warningEl) {
        ...
        const totalElement = colorway.querySelector(
      ...
      if (colorwayTotal < requiredPerColorway) {
        ...
        const totalEl = colorway.querySelector(
          ".techpack-colorway__total-value"
        );
        if (totalEl) {
  ...
  function updateStatusMessage(
    ...
    const messageElement = document.querySelector(
      ".techpack-colorways-message, .quantity-status-message"
    );
    if (messageElement) {
      if (percentage >= 100) {
  ...
  window.recalculateProgress = function () {
    ...
    console.log(
      `Colorways: ${colorways}, Total: ${total}, Required: ${minimum}`
    );
  ...
  function setupCountryDropdown() {
    const countryWrapper = document.querySelector(
      ".techpack-form__country-wrapper"
    );
    if (!countryWrapper) return;

    const countryInput = countryWrapper.querySelector(
      ".techpack-form__country-input"
    );
    const dropdown = countryWrapper.querySelector(".techpack-form__dropdown");
    const toggle = countryWrapper.querySelector(
      ".techpack-form__country-toggle"
    );
    ...
    function populateDropdown(searchTerm = "") {
      ...
      if (!searchTerm) {
        ...
        priorityCountries.forEach((country) => {
          ...
          item.className =
            "techpack-form__dropdown-item techpack-form__dropdown-item--priority";
          item.innerHTML = `
    ...
    function selectCountry(country) {
      ...
      const errorDiv = countryInput.parentElement.querySelector(
    ...
    function openDropdown() {
      ...
      setTimeout(() => {
        const searchInput = dropdown.querySelector(
          ".techpack-form__dropdown-search"
        );
  ...
  function updateHighlight(items, index) {
    items.forEach((item, i) => {
      item.classList.toggle(
        "techpack-form__dropdown-item--highlighted",
        i === index
      );
    });
  ...
  function handleVATFieldVisibility(country) {
    ...
    const vatContainer =
      document.getElementById("vat-ein-group") ||
      document.querySelector(".techpack-form__group--vat");
    const vatInput =
      document.getElementById("vat-ein") ||
      document.querySelector('input[name="vatNumber"]');
    const vatLabel =
      document.getElementById("vat-ein-label") ||
      document.querySelector(
        ".techpack-form__group--vat .techpack-form__label"
      );
    ...
    if (isEuropeanCountry) {
      ...
      if (vatLabel) {
        vatLabel.innerHTML =
          'VAT Number <span class="techpack-form__required">*</span>';
      }
      if (vatStatus) {
      ...
      if (vatLabel) {
        vatLabel.innerHTML =
          'EIN Number <span class="techpack-form__label-status">(optional)</span>';
      }
      if (vatStatus) {
      ...
      console.log(
        `EIN field shown as optional for non-European country: ${country.name}`
      );
    }
  }
  ...
  function validateStep1() {
    ...
    requiredFields.forEach((field) => {
      ...
      if (
        field.name === "country" ||
        field.classList.contains("techpack-form__country-input")
      ) {
        return;
      }
    ...
    const countryInput = form.querySelector(
      '.techpack-form__country-input, input[name="country"]'
    );
  ...
  function validateField(field) {
    ...
    debug.log("Field validation result", {
      name: field.name,
      isValid,
      errorMessage,
    });
  ...
  function displayFieldError(field, isValid, errorMessage) {
    ...
    let errorElement = field.parentNode.querySelector(
      ".techpack-form__error, .field-error-message"
    );
  ...
  function setupProductionTypeListener() {
    const productionTypeSelect = document.querySelector(
      '#production-type, select[name="productionType"]'
    );
  ...
  function updateGarmentInterface(garment, productionType) {
    const garmentTypeSelect = garment.querySelector(
      'select[name="garmentType"]'
    );
    const fabricTypeSelect = garment.querySelector('select[name="fabricType"]');
    const fabricLabel = garment
      .querySelector('select[name="fabricType"]')
      .closest(".techpack-form__group")
      .querySelector(".techpack-form__label");
  ...
  function addFileToList(file) {
    ...
    fileItem.querySelector(".techpack-file__size").textContent = formatFileSize(
      file.size
    );

    // Remove button
    fileItem
      .querySelector(".techpack-file__remove")
  ...
  function validateStep2() {
    ...
    const allFilesHaveTypes = formData.files.every(
      (f) => f.type && f.type.trim() !== ""
    );
    ...
    debug.log("Step 2 validation complete", {
      isValid,
  ...
  function addGarment() {
    ...
    garment.querySelector(".techpack-garment__number").textContent =
      garmentCounter;
  ...
  function setupPrintingMethodsLogic(garment) {
    const checkboxes = garment.querySelectorAll(
      'input[name="printingMethods[]"]'
    );
  ...
  function setupGarmentEventListeners(garment, garmentId) {
    ...
    const printingCheckboxes = garment.querySelectorAll(
      'input[name="printingMethods[]"]'
    );
    printingCheckboxes.forEach((checkbox) => {
      checkbox.addEventListener("change", () => {
        const garmentData = formData.garments.find((g) => g.id === garmentId);
        if (garmentData) {
          const checkedBoxes = garment.querySelectorAll(
            'input[name="printingMethods"]:checked, input[type="checkbox"]:checked'
          );
          garmentData.printingMethods = Array.from(checkedBoxes).map(
            (cb) => cb.value
          );
        }
        validateStep3();
    ...
    allInputs.forEach((input) => {
      if (!input.closest(".techpack-colorway")) {
        // Don't duplicate colorway listeners
  ...
  function addColorway(garmentId) {
    ...
    const colorPreview = colorway.querySelector(
      ".techpack-color-picker__preview"
    );
    ...
    colorPicker.addEventListener("change", function () {
      ...
      const garmentData = formData.garments.find((g) => g.id === garmentId);
      if (garmentData) {
        const colorwayData = garmentData.colorways.find(
          (c) => c.id === colorwayId
        );
        if (colorwayData) {
    ...
    const pantoneInput = colorway.querySelector(
      'input[placeholder*="PANTONE"]'
    );
    if (pantoneInput) {
  ...
  function removeColorway(garmentId, colorwayId) {
    ...
    const colorway = document.querySelector(
      `[data-colorway-id="${colorwayId}"]`
    );
    if (colorway) {
      colorway.remove();
    }
    ...
    const garmentData = formData.garments.find((g) => g.id === garmentId);
    if (garmentData) {
      garmentData.colorways = garmentData.colorways.filter(
        (c) => c.id !== colorwayId
      );
    }
  ...
  function validateQuantityInputs(colorwayId) {
    ...
    const activeSizes = Array.from(qtyInputs).filter(
    ...
    const warningEl =
      colorway.querySelector(".size-distribution-warning") ||
    ...
    if (activeSizes > maxAllowedSizes) {
      ...
      warningEl.style.display = "block";
      warningEl.innerHTML = `📊 Need ${
        requiredPerColorway - colorwayTotal
      } more units. Current: ${activeSizes} sizes, Max allowed: ${maxAllowedSizes} sizes.`;
  ...
  function updateGarmentTotal(garmentId) {
    ...
    const totalElement = garment.querySelector(
  ...
  function updateTotalQuantity() {
    console.log(
      "Progress bar element found:",
      document.getElementById("quantity-progress")
    );
  ...
  function validateStep3() {
    ...
    garments.forEach((garment, index) => {
      ...
      const garmentTypeGroup = garmentTypeSelect
        ? garmentTypeSelect.closest(".techpack-form__group")
        : null;
      const garmentTypeError = garmentTypeGroup
        ? garmentTypeGroup.querySelector(".techpack-form__error")
        : null;
      ...
      if (!garmentTypeSelect || !garmentTypeSelect.value) {
        ...
        console.log(
          "Garment type:",
          garmentTypeSelect ? garmentTypeSelect.value : "NO SELECT FOUND"
        );
        if (garmentTypeGroup)
          garmentTypeGroup.classList.add("techpack-form__group--error");
        if (garmentTypeError)
          garmentTypeError.textContent = "Please select a garment type";
      } else {
      ...
      const fabricSelect = garment.querySelector('select[name="fabricType"]');
      const fabricGroup = fabricSelect
        ? fabricSelect.closest(".techpack-form__group")
        : null;
      const fabricError = fabricGroup
        ? fabricGroup.querySelector(".techpack-form__error")
        : null;
      ...
      if (!fabricSelect || !fabricSelect.value) {
        ...
        console.log(
          "Fabric type:",
          fabricSelect ? fabricSelect.value : "NO SELECT FOUND"
        );
        if (fabricGroup)
          fabricGroup.classList.add("techpack-form__group--error");
        if (fabricError)
          fabricError.textContent = "Please select a fabric type";
      } else {
      ...
      const printingCheckboxes = garment.querySelectorAll(
        'input[name="printingMethods[]"]:checked'
      );
      const printingGroup = garment
        .querySelector(".techpack-form__checkboxes")
        .closest(".techpack-form__group");
      const printingError = printingGroup
        ? printingGroup.querySelector(".techpack-form__error")
        : null;
      ...
      if (printingCheckboxes.length === 0) {
        ...
        console.log(
          "Printing methods:",
          printingCheckboxes.length,
          "checkboxes selected"
        );
        if (printingGroup)
          printingGroup.classList.add("techpack-form__group--error");
        if (printingError)
          printingError.textContent =
            "Please select at least one printing method";
      } else {
        console.log(
          `  ✅ Printing methods: ${
            printingCheckboxes.length
          } selected (${Array.from(printingCheckboxes)
            .map((cb) => cb.value)
            .join(", ")})`
        );
        if (printingGroup) {
        ...
        garmentData.printingMethods = Array.from(printingCheckboxes).map(
          (cb) => cb.value
        );
      }
      ...
      colorways.forEach((colorway, colorwayIndex) => {
        ...
        // Update model
        const colorInput = colorway.querySelector(
        ...
        let colorwayTotal = 0;
        colorway
          .querySelectorAll(".techpack-size-grid__input")
          .forEach((input) => {
        ...
        console.log(
          `    Colorway ${
            colorwayIndex + 1
          }: ${colorwayTotal} units (required: ${requiredPerColorway})`
        );
        ...
        if (colorwayTotal < requiredPerColorway) {
          ...
          console.log(
            `    ❌ Colorway ${
              colorwayIndex + 1
            } below minimum: ${colorwayTotal}/${requiredPerColorway}`
          );
          ...
          console.log(
            `    ✅ Colorway ${
              colorwayIndex + 1
            } meets minimum: ${colorwayTotal}/${requiredPerColorway}`
          );
    ...
    debug.log("Step 3 validation complete", {
      isValid,
  ...
  function populateReviewStep1() {
    ...
    const emailAddress =
      ci.email || ci.emailAddress || ci["contact[email]"] || "N/A";
  ...
  function populateReviewStep2() {
    ...
    if (formData.files.length === 0) {
      container.innerHTML =
        '<p class="techpack-review__empty">No files uploaded</p>';
      return;
    }

    const filesHtml = formData.files
      .map(
        (fileData) => `
  ...
  function populateReviewStep3() {
    ...
    if (formData.garments.length === 0) {
      container.innerHTML =
        '<p class="techpack-review__empty">No garments specified</p>';
      return;
    }

    let totalQuantity = 0;
    const garmentsHtml = formData.garments
      .map((garment, index) => {
        let garmentTotal = 0;

        const colorwaysHtml = garment.colorways
          .map((colorway) => {
            ...
            const colorwayQty = Object.values(colorway.quantities).reduce(
              (sum, qty) => sum + qty,
              0
            );
          ...
          <div class="techpack-review__colorway">
            <div class="techpack-review__colorway-header">
              <div class="techpack-review__color-preview" style="background-color: ${
                colorway.color
              }"></div>
            ...
            <div class="techpack-review__quantities">${
              quantities || "No quantities specified"
            }</div>
          </div>
        `;
          })
        ...
        <div class="techpack-review__garment">
          <div class="techpack-review__garment-header">
            <h4 class="techpack-review__garment-title">Garment ${index + 1}: ${
          garment.type
        }</h4>
          ...
          <div class="techpack-review__garment-details">
            ...
            <div class="techpack-review__detail">
              ...
              <span class="techpack-review__value">${garment.printingMethods.join(
                ", "
              )}</span>
            </div>
          </div>
      ...
      <div class="techpack-success-page">
        ...
        <div class="techpack-success__content">
          ...
          <div class="techpack-success__card">
            ...
            <div class="techpack-success__details">
              <div class="techpack-success__detail-item">
                ...
                <span class="techpack-success__detail-value">TP-${Date.now()
                  .toString()
                  .slice(-8)}</span>
              </div>
              ...
              <div class="techpack-success__detail-item">
                ...
                <span class="techpack-success__detail-value">${
                  formData.files.length
                } ${formData.files.length === 1 ? "file" : "files"}</span>
              </div>
              
              <div class="techpack-success__detail-item">
                <span class="techpack-success__detail-label">Submitted</span>
                <span class="techpack-success__detail-value">${new Date().toLocaleDateString(
                  "en-US",
                  {
                    year: "numeric",
                    month: "long",
          ...
          <div class="techpack-success__actions">
            ...
            <button type="button" class="techpack-btn techpack-btn--ghost" id="save-confirmation-btn">
  ...
  `;
    step4.innerHTML = `
  <!-- your existing HTML with the new button above -->
`;

    // Add event listener after creating the HTML
    setTimeout(() => {
      const saveBtn = document.getElementById("save-confirmation-btn");
      if (saveBtn) {
        saveBtn.addEventListener("click", function () {
          ...
          <body>
            ...
            <div class="details-card">
              ...
              <div class="details-content">
                <div class="detail-item">
                  ...
                  <span class="detail-value">TP-${Date.now()
                    .toString()
                    .slice(-8)}</span>
                </div>
                <div class="detail-item">
                ...
                <div class="detail-item">
                  ...
                  <span class="detail-value">${
                    ...
                    formData.clientInfo?.contactName ||
                    "N/A"
                  }</span>
                </div>
                <div class="detail-item">
                ...
                <div class="detail-item">
                  ...
                  <span class="detail-value">${
                    ...
                    formData.clientInfo?.emailAddress ||
                    "N/A"
                  }</span>
                </div>
                <div class="detail-item">
            ...
            <div class="footer">
              <p>This confirmation was generated on ${new Date().toLocaleDateString(
                "en-US",
                {
                  weekday: "long",
                  year: "numeric",
    ...
    <body>
      ...
      <div class="details-card">
        ...
        <div class="details-content">
          ...
          <div class="detail-item">
            <span class="detail-label">Files Uploaded</span>
            <span class="detail-value">${formData.files.length} ${
