...
class V10_GarmentStudio {
  ...
  populateQuantityStudio() {
    ...
    completedGarments.forEach((garment, index) => {
      ...
      const quantityCard = this.createProfessionalQuantityCard(garment, index);
      if (quantityCard) {
  ...
  renderColorwayRow(garmentId, colorwayData) {
    ...
    const colorwayList = document.getElementById(`colorway-list-${garmentId}`);
    
    if (!sizeGrid || !colorwayList) return;
    
    // Create size grid row
    const gridRow = document.createElement('div');
    gridRow.className = 'size-grid-row';
    gridRow.dataset.colorwayId = colorwayData.id;
    ...
    gridRow.innerHTML = `
      ...
      ${['xxs', 'xs', 's', 'm', 'l', 'xl', 'xxl'].map(size => `
        <input type="number" 
               class="size-input" 
               data-size="${size}" 
               data-colorway-id="${colorwayData.id}"
               value="${colorwayData.quantities[size] || 0}"
               min="0" 
               max="999"
               placeholder="0">
      `).join('')}
    ...
    sizeGrid.appendChild(gridRow);
    
    // Create colorway card for the list
    const colorwayCard = document.createElement('div');
    colorwayCard.className = 'quantity-colorway-card';
    colorwayCard.dataset.colorwayId = colorwayData.id;
    
    colorwayCard.innerHTML = `
      <div class="colorway-color-preview" style="--color: ${colorwayData.color}"></div>
      <div class="colorway-info">
        <div class="colorway-name">${colorwayData.colorName}</div>
        <div class="colorway-details">Click to customize color and name</div>
      </div>
      <div class="colorway-stats">
        <div class="colorway-stat-item">
          <div class="colorway-stat-value" id="colorway-total-${colorwayData.id}">0</div>
          <div class="colorway-stat-label">Total</div>
        </div>
      </div>
      <button type="button" class="v10-colorway-remove" onclick="window.v10GarmentStudio.removeProfessionalColorway('${garmentId}', '${colorwayData.id}')">
        <svg width="16" height="16" viewBox="0 0 16 16" stroke="currentColor" stroke-width="2">
          <path d="M12 4L4 12m0-8l8 8"/>
        </svg>
      </button>
    `;
    
    colorwayList.appendChild(colorwayCard);
    
    // Set up input handlers for this row
    this.setupRowInputHandlers(gridRow, garmentId, colorwayData.id);
  }

  renderExistingColorways(garmentId) {
    const garmentData = V10_State.garments.get(garmentId);
    if (!garmentData || !garmentData.colorways) return;
    
    console.log(`📋 Rendering ${garmentData.colorways.size} existing colorways for garment ${garmentId}`);
    
    // Just render existing colorways without adding new ones
    garmentData.colorways.forEach((colorwayData, colorwayId) => {
      this.renderColorwayRow(garmentId, colorwayData);
    });
  }

  // Clean Colorway Modal Implementation - Using existing v10-submission-option pattern
  showColorwayModal(garmentId) {
    const modal = this.createModalOverlay('Add Colorway', 'v10-colorway-modal', {
      maxWidth: '500px'
    });
    
    modal.body.innerHTML = `
      <div class="v10-modal-description" style="margin-bottom: var(--v10-space-6);">
        <p style="color: var(--v10-text-secondary); margin: 0; text-align: center;">Choose your colorway type:</p>
      </div>
      <div class="v10-submission-options-grid">
        <button type="button" class="v10-submission-option" data-choice="stock">
          <div class="v10-submission-card">
            <div class="v10-submission-icon">
            ...
            <div class="v10-submission-content">
              <h3 class="v10-submission-title">Stock Fabric Color</h3>
              <p class="v10-submission-description">Pick this option if your sample was done in a stock fabric colorway</p>
              <div class="v10-submission-badges">
                <span class="v10-submission-badge">Stock Colors</span>
                <span class="v10-submission-badge">Faster Turnaround</span>
              </div>
            </div>
          </div>
        </button>
        <button type="button" class="v10-submission-option" data-choice="custom">
          <div class="v10-submission-card">
            <div class="v10-submission-icon">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                <path d="M21 12c-1 0-3-1-3-3s2-3 3-3 3 1 3 3-2 3-3 3" stroke-width="2"/>
                <path d="M9 12c1 0 3 1 3 3s-2 3-3 3-3-1-3-3 2-3 3-3" stroke-width="2"/>
                <path d="M13 5l4.5 4.5" stroke-width="2"/>
                <path d="M5.5 18.5L10 14" stroke-width="2"/>
              </svg>
            </div>
            <div class="v10-submission-content">
              <h3 class="v10-submission-title">Custom Colorways & Patterns</h3>
              <p class="v10-submission-description">Pick this option if your sample was done in a custom colorway OR has a pattern</p>
              <div class="v10-submission-badges">
                <span class="v10-submission-badge">Pantone TCX/TPX</span>
                <span class="v10-submission-badge">LabDip Color Code</span>
              </div>
            </div>
          </div>
        </button>
    ...
    modal.footer.innerHTML = `
      <button type="button" class="v10-btn v10-btn--secondary" data-action="cancel">Cancel</button>
    `;
    
    // Handle choice selection
    const choiceButtons = modal.body.querySelectorAll('[data-choice]');
    choiceButtons.forEach(button => {
      button.addEventListener('click', () => {
        const choice = button.dataset.choice;
        this.closeModal(modal.overlay);
        if (choice === 'stock') {
          this.showStockColorModal(garmentId);
        } else if (choice === 'custom') {
          this.showCustomColorModal(garmentId);  
        }
      });
    });
  ...
  showStockColorModal(garmentId) {
    const modal = this.createModalOverlay('Stock Fabric Color', '', {
      maxWidth: '600px'
    });
    
    modal.body.innerHTML = `
      <div class="v10-stock-colors-container">
        <div class="v10-stock-section">
          <h4 class="v10-stock-section-title">Standard Colors</h4>
          <div class="v10-stock-colors-grid">
            <button type="button" class="v10-stock-color-button" data-color="black" data-hex="#000000">
              <span>Black</span>
            </button>
            <button type="button" class="v10-stock-color-button" data-color="off-white" data-hex="#f8f8ff">
              <span>Off-White</span>
            </button>
            <button type="button" class="v10-stock-color-button" data-color="white" data-hex="#ffffff">
              <span>White</span>
            </button>
          </div>
        </div>
        <div class="v10-stock-section">
          <button type="button" class="v10-submission-option" data-toggle="sample-picker">
            <div class="v10-submission-card">
              <div class="v10-submission-icon">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  <rect x="3" y="3" width="18" height="18" rx="2" stroke-width="2"/>
              ...
              <div class="v10-submission-content">
                <h3 class="v10-submission-title">Stock Color from Sample</h3>
                <p class="v10-submission-description">Match color from a sample you've received</p>
                <div class="v10-submission-badges">
                  <span class="v10-submission-badge">Sample Matching</span>
                  <span class="v10-submission-badge">Custom Colors</span>
                </div>
              </div>
            </div>
          </button>
          <div class="v10-color-picker-section" data-section="sample-picker">
            <!-- Exact Copy from Custom Colorways - Always Visible -->
            <div class="v10-custom-color-reference-card">
              <div class="v10-custom-color-reference-title">Visual Color Reference</div>
              <div class="v10-custom-color-picker-row">
                <input type="color" class="v10-color-picker-input" value="#00832D" data-color-input>
              </div>
              <div class="v10-custom-color-info-card">
                <h4 class="v10-custom-color-name-large" data-color-name>Jolly green</h4>
              </div>
            </div>
          </div>
        </div>
    ...
    modal.footer.innerHTML = `
      <button type="button" class="v10-btn v10-btn--secondary" data-action="back">Back</button>
      <button type="button" class="v10-btn v10-btn--primary" data-action="add-color" disabled>Add Color</button>
    `;
    
    let selectedColorData = null;
    
    // Handle standard color selection
    const colorButtons = modal.body.querySelectorAll('[data-color]');
    colorButtons.forEach(button => {
      button.addEventListener('click', () => {
        ...
        modal.footer.querySelector('[data-action="add-color"]').disabled = false;
      });
    });
    
    // Handle sample section toggle  
    const sampleToggle = modal.body.querySelector('[data-toggle="sample-picker"]');
    const sampleSection = modal.body.querySelector('[data-section="sample-picker"]');
    
    sampleToggle.addEventListener('click', () => {
      const isActive = sampleSection.classList.contains('active');
      
      if (isActive) {
        sampleSection.classList.remove('active');
        
        // Hide color reference card
        const colorReferenceCard = modal.body.querySelector('.v10-custom-color-reference-card');
        if (colorReferenceCard) {
          colorReferenceCard.classList.remove('active');
        }
      } else {
        sampleSection.classList.add('active');
        
        // Make color reference card visible
        const colorReferenceCard = modal.body.querySelector('.v10-custom-color-reference-card');
        if (colorReferenceCard) {
          colorReferenceCard.classList.add('active');
        }
        
        // Clear standard color selections when using color picker
        colorButtons.forEach(btn => btn.classList.remove('selected'));
        selectedColorData = null;
        modal.footer.querySelector('[data-action="add-color"]').disabled = false;
        
        // Initialize color picker immediately (no text input required)
        initializeDefaultColor();
      }
    });
    
    // Handle color picker - exact same logic as Custom Colorways
    const colorInput = modal.body.querySelector('[data-color-input]');
    const colorNameDisplay = modal.body.querySelector('[data-color-name]');
    
    // Initialize default color
    const initializeDefaultColor = () => {
      const defaultColor = '#00832D';
      const defaultClosestColors = V10_Utils.findClosestPantoneColors(defaultColor, 1);
      const defaultClosest = defaultClosestColors.length > 0 ? defaultClosestColors[0] : null;
      if (defaultClosest) {
        colorNameDisplay.textContent = defaultClosest.name;
        
        // Set initial selected color data
        selectedColorData = {
          type: 'sample',
          name: defaultClosest.name,
          hex: defaultColor,
          pantone: defaultClosest.code || null
        };
      }
      // Enable Add Color button immediately since color picker is ready
      modal.footer.querySelector('[data-action="add-color"]').disabled = false;
    };
    ...
    colorInput.addEventListener('input', (e) => {
      ...
      const closestColor = closestColors.length > 0 ? closestColors[0] : null;
      
      if (closestColor && closestColor.name) {
        colorNameDisplay.textContent = closestColor.name;
        
        // Update selected color data for sample color picker
        selectedColorData = {
          type: 'sample',
          name: closestColor.name,
          hex: hex,
          pantone: closestColor.code || null
        };
    ...
    // Handle actions
    modal.footer.querySelector('[data-action="back"]').addEventListener('click', () => {
      this.closeModal(modal.overlay);
      this.showColorwayModal(garmentId);
    });
    
    modal.footer.querySelector('[data-action="add-color"]').addEventListener('click', () => {
      if (selectedColorData) {
        this.addProfessionalColorway(garmentId, selectedColorData.hex, selectedColorData.name);
        this.closeModal(modal.overlay);
      }
    });
  }

  showCustomColorModal(garmentId) {
    const modal = this.createModalOverlay('Custom Colorways & Patterns', '', {
      maxWidth: '500px'
    });
    
    modal.body.innerHTML = `
      <div class="v10-custom-modal-container">
        <!-- Input Card -->
        <div class="v10-custom-input-card">
          <label class="v10-custom-input-label">Pantone Code or Custom Reference</label>
          <div class="v10-custom-input-row">
            <input type="text" class="v10-input" data-pantone-input placeholder="e.g., 18-1664 TPX, Custom Blend #1" style="flex: 1;">
            <button type="button" class="v10-btn v10-btn--primary" data-action="validate">Validate</button>
          </div>
        </div>
        
        <!-- Color Reference Card (Hidden Initially) -->
        <div class="v10-custom-color-reference-card" data-color-section>
          <div class="v10-custom-color-reference-title">Visual Color Reference</div>
          <div class="v10-custom-color-picker-row">
            <input type="color" class="v10-color-picker-input" value="#B33A3A" data-color-input>
          </div>
          <div class="v10-custom-color-info-card">
            <h4 class="v10-custom-color-name-large" data-color-name>Bossa nova</h4>
          </div>
        </div>
      </div>
    `;
    ...
    modal.footer.innerHTML = `
      <button type="button" class="v10-btn v10-btn--secondary" data-action="back">Back</button>
      <button type="button" class="v10-btn v10-btn--primary" data-action="add-color">Add Color</button>
    `;
    
    // Handle color picker
    const colorInput = modal.body.querySelector('[data-color-input]');
    const colorNameDisplay = modal.body.querySelector('[data-color-name]');
    const pantoneInput = modal.body.querySelector('[data-pantone-input]');
    const colorSection = modal.body.querySelector('[data-color-section]');
    
    // Initialize default color when section becomes visible
    const initializeDefaultColor = () => {
      const defaultColor = '#B33A3A';
      const defaultClosestColors = V10_Utils.findClosestPantoneColors(defaultColor, 1);
      const defaultClosest = defaultClosestColors.length > 0 ? defaultClosestColors[0] : null;
      if (defaultClosest) {
        colorNameDisplay.textContent = defaultClosest.name;
    ...
    // Show color picker section only on Enter key or Validate button
    const showColorSection = () => {
      const hasText = pantoneInput.value.trim().length > 0;
      if (hasText) {
    ...
    modal.body.querySelector('[data-action="validate"]').addEventListener('click', () => {
      showColorSection();
    });
    ...
    modal.footer.querySelector('[data-action="add-color"]').addEventListener('click', () => {
      const pantoneValue = pantoneInput.value.trim();
      const hex = colorInput.value;
      const colorName = colorNameDisplay.textContent;
      
      const colorData = {
        type: 'custom',
        pantone: pantoneValue || 'Custom',
        name: colorName,
        hex: hex
      };
      
      this.addProfessionalColorway(garmentId, colorData.hex, colorData.name);
      this.closeModal(modal.overlay);
    });
  }

  // Helper method to create modal overlay with proper structure
  createModalOverlay(title, modalClass = '', options = {}) {
    const overlay = document.createElement('div');
    overlay.className = 'v10-modal-overlay';
    ...
    modal.className = `v10-modal ${modalClass}`;
    if (options.maxWidth) {
      modal.style.maxWidth = options.maxWidth;
    }
    
    const header = document.createElement('div');
    header.className = 'v10-modal-header';
    header.innerHTML = `
      <div class="v10-modal-header-spacer"></div>
      <h3 class="v10-modal-title v10-text-center">${title}</h3>
    ...
    body.className = 'v10-modal-body';
    
    const footer = document.createElement('div');
    footer.className = 'v10-modal-footer';
    ...
    // Handle close events
    header.querySelector('.v10-modal-close').addEventListener('click', () => {
  ...
  updateColorwayCount(garmentId) {
    const countElement = document.getElementById(`colorway-count-${garmentId}`);
    if (countElement) {
      const count = this.getColorwayCount(garmentId);
      countElement.textContent = `${count} colorway${count !== 1 ? 's' : ''}`;
    }
  }

  updateGarmentStats(garmentId) {
    const garmentData = V10_State.garments.get(garmentId);
    if (!garmentData) return;

    // Update total units
    const totalElement = document.getElementById(`total-${garmentId}`);
    if (totalElement) {
      let total = 0;
      if (garmentData.colorways) {
        garmentData.colorways.forEach(colorway => {
          if (colorway.quantities) {
            total += Object.values(colorway.quantities).reduce((sum, qty) => sum + (parseInt(qty) || 0), 0);
          }
        });
      }
      totalElement.textContent = total;
    ...
    // Update colors count
    const colorsElement = document.getElementById(`colors-${garmentId}`);
    if (colorsElement) {
      colorsElement.textContent = this.getColorwayCount(garmentId);
    }

    // Update sizes count (count of sizes with quantities > 0)
    const sizesElement = document.getElementById(`sizes-${garmentId}`);
    if (sizesElement) {
      let activeSizes = new Set();
      if (garmentData.colorways) {
        garmentData.colorways.forEach(colorway => {
          if (colorway.quantities) {
            Object.entries(colorway.quantities).forEach(([size, qty]) => {
              if (parseInt(qty) > 0) {
      ...
      sizesElement.textContent = activeSizes.size;
    }
  }

  handleSizeInputChange(input, garmentId, colorwayId) {
    const size = input.dataset.size;
    const value = parseInt(input.value) || 0;
  ...
  setupRowInputHandlers(row, garmentId, colorwayId) {
    const inputs = row.querySelectorAll('.size-input');
    
    inputs.forEach(input => {
      input.addEventListener('input', () => {
        this.handleSizeInputChange(input, garmentId, colorwayId);
      });
  ...
  handleSizeInputChange(input, garmentId, colorwayId) {
    ...
    const garmentData = V10_State.garments.get(garmentId);
    if (garmentData && garmentData.colorways && garmentData.colorways.has(colorwayId)) {
      garmentData.colorways.get(colorwayId).quantities[size] = value;
    }
    ...
    // Update stats
    this.updateColorwayStats(garmentId, colorwayId);
    this.updateGarmentStats(garmentId);
    this.updateOverallStats();
    
    console.log(`📊 Updated ${size} size for colorway ${colorwayId}: ${value}`);
  }

  removeProfessionalColorway(garmentId, colorwayId) {
    const garmentData = V10_State.garments.get(garmentId);
    ...
    // Remove from DOM
    const gridRow = document.querySelector(`[data-colorway-id="${colorwayId}"]`);
    const colorwayCard = document.querySelector(`.quantity-colorway-card[data-colorway-id="${colorwayId}"]`);
    
    if (gridRow) gridRow.remove();
    if (colorwayCard) colorwayCard.remove();
    
    this.updateColorwayCount(garmentId);
    this.updateGarmentStats(garmentId);
    this.updateOverallStats();
    
    console.log(`🗑️ Removed colorway ${colorwayId} from garment ${garmentId}`);
  }

  updateColorwayStats(garmentId, colorwayId) {
    const garmentData = V10_State.garments.get(garmentId);
    if (!garmentData || !garmentData.colorways) return;
    
    const colorwayData = garmentData.colorways.get(colorwayId);
    if (!colorwayData) return;
    
    const total = Object.values(colorwayData.quantities).reduce((sum, qty) => sum + (qty || 0), 0);
    
    const totalElement = document.getElementById(`colorway-total-${colorwayId}`);
    if (totalElement) {
      totalElement.textContent = total;
    }
  }
  ...
  updateGarmentStats(garmentId) {
    ...
    let totalQuantity = 0;
    let activeSizes = new Set();
    const colorwayCount = garmentData.colorways ? garmentData.colorways.size : 0;
    ...
    const totalEl = document.getElementById(`total-${garmentId}`);
    const colorsEl = document.getElementById(`colors-${garmentId}`);
    const sizesEl = document.getElementById(`sizes-${garmentId}`);
    
    if (totalEl) totalEl.textContent = totalQuantity;
    if (colorsEl) colorsEl.textContent = colorwayCount;
    if (sizesEl) sizesEl.textContent = activeSizes.size;
  }

  updateColorwayCount(garmentId) {
    const garmentData = V10_State.garments.get(garmentId);
    const count = garmentData && garmentData.colorways ? garmentData.colorways.size : 0;
    
    const countEl = document.getElementById(`colorway-count-${garmentId}`);
    if (countEl) {
      countEl.textContent = `${count} colorway${count !== 1 ? 's' : ''}`;
    }
  }
  ...
  updateOverallStats() {
    ...
    V10_State.garments.forEach(garment => {
      if (garment.colorways) {
        totalGarments++;
        totalColorways += garment.colorways.size;
        
        garment.colorways.forEach(colorway => {
          totalUnits += Object.values(colorway.quantities).reduce((sum, qty) => sum + (qty || 0), 0);
        });
      }
    });
    
    // Update main stats
    const totalUnitsEl = document.getElementById('total-production-quantity');
    const totalGarmentsEl = document.getElementById('total-garments-count');
    const totalColorwaysEl = document.getElementById('total-colorways-count');
    const progressEl = document.getElementById('production-quantity-progress');
    const progressTextEl = document.getElementById('quantity-progress-status');
    const progressPercentageEl = document.getElementById('quantity-progress-percentage');
    
    if (totalUnitsEl) totalUnitsEl.textContent = totalUnits;
    if (totalGarmentsEl) totalGarmentsEl.textContent = totalGarments;
    if (totalColorwaysEl) totalColorwaysEl.textContent = totalColorways;
    ...
    if (progressTextEl) progressTextEl.textContent = `${totalUnits} / ${minRequired} minimum units`;
  ...
  // Professional Size Input Setup
  setupProfessionalSizeInputs(card, garment) {
    // Set up global input handlers (already handled in renderColorwayRow)
    console.log(`📊 Professional size inputs setup for garment ${garment.id}`);
  }

  setupRealtimeStatsUpdates(card, garment) {
    // Set up real-time updates for statistics
    console.log(`📈 Real-time stats setup for garment ${garment.id}`);
  }
  ...
  initializeBulkTools() {
    ...
    const presetDropdown = document.getElementById('preset-dropdown-menu');
    
    if (clearAllBtn) {
      clearAllBtn.addEventListener('click', () => this.clearAllQuantities());
    }
    
    if (applyPresetBtn && presetDropdown) {
      ...
      document.addEventListener('click', (e) => {
        if (!e.target.closest('.bulk-preset-dropdown')) {
    ...
    console.log('🛠️ Bulk tools initialized');
  }
  ...
  clearAllQuantities() {
    const sizeInputs = document.querySelectorAll('#garment-quantities-container .size-input');
  ...
  applyQuantityPreset(presetType) {
    const presets = {
      'bell-curve': [2, 6, 8, 10, 8, 6, 2],
      'medium-heavy': [3, 5, 8, 12, 8, 5, 3],
      'large-heavy': [2, 4, 6, 8, 12, 10, 8],
      'equal-split': [6, 6, 6, 6, 6, 6, 6]
    };
    
    const distribution = presets[presetType];
    if (!distribution) return;
    
    const sizeNames = ['xxs', 'xs', 's', 'm', 'l', 'xl', 'xxl'];
    
    // Apply to all colorways
    const sizeGrids = document.querySelectorAll('.size-grid-content .size-grid-row');
    
    sizeGrids.forEach(row => {
      distribution.forEach((qty, index) => {
        const input = row.querySelector(`[data-size="${sizeNames[index]}"]`);
        if (input) {
          input.value = qty;
          input.classList.add('has-value');
    ...
    console.log(`📊 Applied ${presetType} preset to all colorways`);
  }
