      ...
      switch (actionType) {
        ...
        case 'add-stock-color':
          if (window.TechPackV11.BulkStep4) {
            ...
            window.TechPackV11.BulkStep4.startAddingStockColor(garmentId);
          }
          break;
        ...
        case 'select-stock-color':
          ...
          if (element.getAttribute('data-garment-id')) {
            // Sample Step 3B (has garment-id attribute)
            ...
            // Bulk Step 4 (uses data-color and data-hex)
            if (window.TechPackV11.BulkStep4) {
        ...
        case 'delete-colorway':
          if (window.TechPackV11.BulkStep4) {
            const garmentId = element.getAttribute('data-garment-id');
            const colorwayId = element.getAttribute('data-colorway-id');
            window.TechPackV11.BulkStep4.deleteColorway(garmentId, colorwayId);
          }
          break;
        ...
        case 'expand-stock-colors':
          if (window.TechPackV11.Step3B) {
            const garmentId = element.getAttribute('data-garment-id');
            window.TechPackV11.Step3B.expandStockColors(garmentId);
          }
          break;
        ...
        case 'expand-techpack-reference':
          if (window.TechPackV11.Step3B) {
            ...
            window.TechPackV11.Step3B.expandTechPackReference(garmentId, isCotton);
          }
          break;
        ...
        case 'assign-techpack-reference':
          if (window.TechPackV11.Step3B) {
            const garmentId = element.getAttribute('data-garment-id');
            window.TechPackV11.Step3B.assignTechPackReference(garmentId);
          }
          break;
        ...
        case 'delete-garment-color':
          if (window.TechPackV11.Step3B) {
            const garmentId = element.getAttribute('data-garment-id');
            const garment = window.TechPackV11.Step3B.garments.find(g => g.id === garmentId);

            if (garment && confirm('Permanently delete this color? It will be removed from everywhere.')) {
              // If Pantone, remove from Lab Dip Cart too
              if (garment.colorType === 'pantone' && garment.pantoneColor && garment.pantoneColor.labDipId) {
                const labDipIndex = window.TechPackV11.Step3B.labDipCart.findIndex(
                  item => item.id === garment.pantoneColor.labDipId
                );

                if (labDipIndex !== -1) {
                  console.log('[Actions] Deleting Pantone from Lab Dip Cart:', garment.pantoneColor.labDipId);
                  window.TechPackV11.Step3B.labDipCart.splice(labDipIndex, 1);
                  window.TechPackV11.Step3B.renderLabDipCart();
                }
              }

              // Clear color assignment from garment
              garment.colorAssigned = false;
              garment.colorType = null;
              garment.color = null;
              garment.hex = null;
        ...
        case 'edit-garment':
          // Navigate back to Step 3A to edit garment type/fabric
          if (window.TechPackV11 && window.TechPackV11.Navigation) {
            console.log('[Actions] Navigating back to Step 3A to edit garment');
            window.TechPackV11.Navigation.navigateToStep(3);
          }
          break;

        case 'delete-garment':
          // Delete entire garment from list
          if (window.TechPackV11.Step3B) {
            const garmentId = element.getAttribute('data-garment-id');
            ...
            if (garmentIndex !== -1) {
              ...
              if (confirm(`Delete ${garment.type}? This will remove it completely from your list.`)) {
                ...
                if (garment.colorType === 'pantone' && garment.pantoneColor) {
                  ...
                  console.log('[Actions] Returned Pantone to cart before deleting garment');
                }

                // Remove from garments array
                window.TechPackV11.Step3B.garments.splice(garmentIndex, 1);

                console.log('[Actions] Deleted garment:', garment.type);
        ...
        // Modal search handlers (fixed IDs)
        case 'modal-search-pantone':
        case 'modal-search-pantone-button':
          if (window.TechPackV11.ColorStudio) {
            const searchInput = document.getElementById('modalPantoneSearch');
            if (searchInput && searchInput.value) {
              window.TechPackV11.ColorStudio.searchPantone(searchInput.value);
            }
          }
          break;
